...
app.post('/api/generate', authMiddleware, async (req, res) => {
    try {
        ...
        // **FIX**: Add robust check for safety blocks or empty responses from Gemini
        if (!response.candidates || !response.candidates[0] || !response.candidates[0].content || !response.candidates[0].content.parts) {
            console.error("Gemini Response Blocked or Malformed:", JSON.stringify(response, null, 2));
            return res.status(500).json({ message: 'Image generation failed. The request may have been blocked for safety reasons or another issue occurred.' });
        }

        for (const part of response.candidates[0].content.parts) {
            if (part.inlineData) {
                // Do not deduct credits from admin
                if (!user.isAdmin) {
                   user.credits -= 1;
                   await user.save();
                }
