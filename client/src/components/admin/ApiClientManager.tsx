...
import { createClient, updateClientAsAdmin } from '../../services/apiService';
...
export const ApiClientManager: React.FC<ApiClientManagerProps> = ({ clients, setClients, usageLogs, setUsageLogs }) => {
  ...
  const handleAddClient = async (e: React.FormEvent) => {
    ...
    try {
        const newClientData = {
            ...
            passwordHash: newClientPassword, // The backend will hash this
  ...
  const handleUpdateClient = async (updatedClient: ClientUser) => {
    try {
        const result = await updateClientAsAdmin(updatedClient);
  ...
  const toggleClientStatus = (client: ClientUser) => {
    const updatedClient = { ...client, status: client.status === 'active' ? 'revoked' as const : 'active' as const };
      ...
      {showAddForm && (
        <form onSubmit={handleAddClient} className="bg-gray-50 p-4 rounded-lg border mb-6 space-y-4">
            ...
            <div>
                ...
                <input type="password" id="clientPassword" value={newClientPassword} onChange={e => setNewClientPassword(e.target.value)} className="mt-1 w-full p-2 border rounded-md" required />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                  <label htmlFor="clientCredits">Initial Credits</label>
               ...
               <div>
                <label htmlFor="clientCreditLimit">Credit Limit</label>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full bg-white border-separate" style={{ borderSpacing: '0 0.5rem' }}>
          <thead className="bg-gray-50 hidden md:table-header-group">
            <tr>
              <th className="p-3 text-left text-xs font-semibold text-gray-500 uppercase rounded-l-lg">Client</th>
              <th className="p-3 text-left text-xs font-semibold text-gray-500 uppercase">API Key</th>
              <th className="p-3 text-left text-xs font-semibold text-gray-500 uppercase">Status</th>
              <th className="p-3 text-left text-xs font-semibold text-gray-500 uppercase">Credits</th>
              <th className="p-3 text-left text-xs font-semibold text-gray-500 uppercase rounded-r-lg">Actions</th>
            </tr>
          </thead>
              ...
              >
                ...
                <td className="p-3 whitespace-nowrap md:rounded-r-lg block md:table-cell">
                    <div className="flex items-center gap-4 text-sm font-medium flex-wrap">
                        ...
                        <button onClick={(e) => { e.stopPropagation(); toggleClientStatus(client); }} className={`${client.status === 'active' ? 'text-red-600 hover:text-red-800' : 'text-green-600 hover:text-green-800'}`}>
